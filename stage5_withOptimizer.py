...
import stage3_trainRegressor_withOptimizer
...
from optimizers.NSGA_II import *
...
def main_withOptimizer(info, prepared_data, stages_data, trained_models):
    ...
    SOO_optimizerName = info['SOO_optimizerName']
    MOO_optimizerName = info['MOO_optimizerName']
    ...
    ranksLoadings_stages = stages_data['ranksLoadings_stages']
    ...
    optimizeLoadings_stages = stages_data['optimizeLoadings_stages']

    sim = SIM(info)
    sim.fileIndex = 0 # The file index is just 0. 
    # After an iteration, the code will delete the simulation folder on CSC and create a new one again
    # This aims to prevent using over disk memory so the optimization process can run without interruption  
    ...
    if optimizeStrategy == "SOO_withOptimizer":
        if SOO_optimizerName == "GA":
            fullOptimizerName = "Genetic Algorithm"
            abbreOptimizerName = "GA"
            optimizer = GA(info_optimize, prepared_data, trained_models)
        # if SOO_optimizerName == "BOSS":
        #     fullOptimizerName = "Bayesian Optimization Structural Search"
        #     abbreOptimizerName = "BOSS"
        #     optimizer = BOSS(info_optimize, prepared_data, trained_models)
    elif optimizeStrategy == "MOO_withOptimizer":
        if MOO_optimizerName == "NSGA_II":
            fullOptimizerName = "Non-Dominated Sorting Genetic Algorithm II"
            abbreOptimizerName = "NSGA_II"
            optimizer = NSGA_II(info_optimize, prepared_data, trained_models)
    #print(optimizeStrategy)
    #print(MOO_optimizerName)
    printLog(f"The optimization strategy is {optimizeStrategy}\n", logPath)
    ...
    # The assumption is that there is always a linear loading in the loadings list
    ...
    for stageNumber in stages:
        ...
        ranksLoadings = ranksLoadings_stages[stageNumber]
        weightsConstitutive = weightsConstitutive_stages[stageNumber]
        optimizeLoadings = optimizeLoadings_stages[stageNumber]
        ...
        #####################################################################
            ...
            if stageNumber == 2:
                allLoadingsSatisfied, notSatisfiedLoadings = False, optimize_loadings
            else:
            ...
            while not allLoadingsSatisfied: 
                ...
                while not converging:
                    ...
                    if optimizeStrategy == "SOO_withOptimizer":
                        optimizer.initializeOptimizer(default_params, optimizeParams, optimizeLoadings, lossFunction, weightsLoadings, weightsConstitutive)
                    elif optimizeStrategy == "MOO_withOptimizer":
                        optimizer.initializeOptimizer(default_params, optimizeParams, optimizeLoadings, lossFunction, ranksLoadings, weightsConstitutive)
                    ...
                    while bestParams['solution_tuple'] in reverse_combined_loadings_interpolateCurves.keys():
                        ...
                        printLog(f"Searching time by {abbreOptimizerName}: {round(end - start, 2)}s\n\n", logPath)
                        printLog(f"The best candidate parameters found by {abbreOptimizerName}\n", logPath)
                        printLog("The current result parameters\n", logPath)
                        ...
                        printLog(f"Parameters already probed. {abbreOptimizerName} needs to run again to obtain new parameters\n", logPath)
                        ...
                        retrainingRegressor(optimizeType, loadings, regressors, scaler, combined_loadings_interpolateCurves, info)
                ...
                for loading in loadings:
                    predictedParams = scaler.transform(np.array(list(stage_curve["parameters_dict"].values())).reshape(1, -1))
...
if __name__ == '__main__':
    ...
    trained_models = stage3_trainRegressor_withOptimizer.main_trainRegressor_withOptimizer(info, prepared_data)
    stages_data = stage4_stages_analysis.main_stagesAnalysis(info, prepared_data)
    main_withOptimizer(info, prepared_data, stages_data, trained_models)