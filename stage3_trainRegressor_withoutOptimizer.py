...
def main_trainRegressor_withoutOptimizer(info, prepared_data, logging):
    ...
    if logging:
        ...
        printLog(f"ANN model: (stress values at interpolating strain points) -> (parameters)\n", logPath)
        
        numberOfHiddenLayers = trainingParams['numberOfHiddenLayers']
        hiddenSize = trainingParams['hiddenSize']
        
        dropoutRate = trainingParams['dropoutRate']
        learning_rate = trainingParams['learning_rate']
        weight_decay = trainingParams['weight_decay']
        step_size = trainingParams['step_size']
        gamma = trainingParams['gamma']
        tolerance = trainingParams['tolerance']
        patience = trainingParams['patience']
        test_ratio = trainingParams['test_ratio']
    ...
    hiddenSizes = np.round((numberOfLoadings * 0.1 + 1) * np.linspace(inputSize, outputSize, trainingParams['numberOfHiddenLayers']+2)[1:-1]).astype(int)
    trainingParams['hiddenSizes'] = hiddenSizes
    
    regressors[loading] = NeuralNetwork(inputSize, outputSize, trainingParams).to(device)
    regressors[loading].trainRegressor(stressLabels, paramFeatures, trainingParams, True)
...
if __name__ == '__main__':
    ...
    main_trainRegressor_withoutOptimizer(info, prepared_data, logging=True)